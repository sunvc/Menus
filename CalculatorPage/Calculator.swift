                    ...
                    ForEach(results, id: \.id) { item in
                        VStack(spacing: 0) {
                            ...
                            ResultView(result: item) {
                                ...
                                self.inputText = Manager.appendOrReplaceLastToken(
                                    in: tokens,
                                    with: item.result
                                ).joined()
                            }
    ...
    func button(_ text: String) {
        ...
        case "/", "*", "+", "-":
            if let last = inputText.last, Int(String(last)) != nil {
                ...
                inputText = String(inputText.dropLast()) + text
            }
            var texts = Manager.tokenizeMathExpression(inputText)

            inputText = texts.compactMap { item in
                if ["/", "*", "+", "-"].contains(item) {
                    return item
                }
                if texts.last == item {
            ...
            inputText = Manager.removeZeroAndOperators(from: texts).joined()
            return
        case ".":
        ...
        case "=":
            ...
            if let last = texts.last, ["/", "*", "+", "-"].contains(last) {
                inputText = texts.dropLast().joined()
            }
            texts = Manager.tokenizeMathExpression(inputText)
