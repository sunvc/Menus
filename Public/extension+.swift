...
extension Color {
    init(from comm: String) {
        // 判断是否是十六进制颜色代码
        if comm.hasPrefix("#") || comm
            .range(of: "^[0-9A-Fa-f]{3,8}$", options: .regularExpression) != nil
        {
            self = Color(hex: comm)
        } else {
            // 默认作为资源名称初始化
...
extension View {
    var deviceCornerRadius: CGFloat {
        ...
        if let screen = (UIApplication.shared.connectedScenes.first as? UIWindowScene)?.windows
            .first?.screen
        {
